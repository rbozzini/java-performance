...
public class ReentrantLockEsempio {
	...
	public int conta() {
		System.out.println(
				"Il thread " + Thread.currentThread().getName() + " ha richiesto di incrementare il contatore");
	...
	public void somma() {
		System.out.println("Il thread " + Thread.currentThread().getName()
				+ " ha richiesto di visualizzare la somma dei contatori");
		...
		if (istanzaLock.tryLock()) {
			...
			System.out.println(
					"------ Il thread che ha il lock sull'oggetto è " + Thread.currentThread().getName() + " --------");
		}
	}
