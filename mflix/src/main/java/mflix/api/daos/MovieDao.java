...
//		pipeline.add(match);
		// Ticket: Get Comments - implement the lookup stage that allows the
		// comments to
...
//		Bson queryFilter = new Document("countries", new Document("$all", Arrays.asList(country)));
	...
	public List<Document> getMoviesByGenre(String sortKey, int limit, int skip, String... genres) {
		...
		Bson sort = Sorts.descending(sortKey);
		List<Document> movies = new ArrayList<>();
		// Ticket: Paging - implement the necessary cursor methods to support
		// simple
	...
	public List<Document> getMoviesCastFaceted(int limit, int skip, String... cast) {
		...
		// Ticket: Faceted Search - build the aggregation pipeline by adding all
		// stages in the
		// correct order
