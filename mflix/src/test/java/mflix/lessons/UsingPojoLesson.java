...
public class UsingPojoLesson extends AbstractLesson {
	...
	/*
	 * In this lesson we are going to discuss the options available to us when
	 * reading and parsing through a document using the Java Driver, We will
	 * contrast these varying approaches to find the one that suits us best.
	 *
	 * For the purposes of this lesson we will create an actors collection, that
	 * contains documents corresponding to each actor.
	 *
	 * In our first example we want to create a basic case where the structure of
	 * the Actor class mimics a document from the actors collection, so we will call
	 * this class ActorBasic.
	 * 
	 * @see ActorBasic
	 *
	 *
	 * Before we start exploring, we need to set up our collection and throw a
	 * couple of test documents in there.
	 */
	...
	public void setUp() throws Exception {
		...
		/*
		 * Now we can create two document objects and fill in each field value.
		 */
		...
		/*
		 * Great! We now have two Document Objects with data, that are ready to be
		 * written to the actors collection. For that we create an array of documents
		 * and append both Bruce and Natalie to that array. Finally we can use the
		 * insertMany method to add both documents to the actors database.
		 */
	 ...
	 * correspondence between the document field data types and the class properties
	 * data types. This method uses Pojo code
	 *
	...
	public void testReadUsingPojo() {
		...
		/*
		 * The first thing that we need here is a pojoCodecRegistry. You can think of
		 * the registry as a "factory" of Codecs, where Codecs are an abstraction that
		 * determines how BSON data is converted and into what type. There are default
		 * and custom codecs, so let's start with a default one and see what it can do
		 * for us.
		 *
		 * The mechanism for POJO support is via the PojoCodecProvider which provides a
		 * builder for configuring how and what POJOs to support. The builder allows
		 * registering of classes and packages, with a helpful setting that directs the
		 * provider to automatically try and handle any POJO it sees.
		 *
		 * Here we are instantiating a codec registry and telling it to use the default
		 * pojo provider, building it with an automatic setting, which uses type
		 * introspection
		 * 
		 * @https://en.wikipedia.org/wiki/Type_introspection to determine the properties
		 * of the given POJO and accurately apply the default Codec.
		 *
		 */
	 ...
	 *
	 * Well, our ActorBasic object is the only point of reference our CodecRegistry
	 * has and it seems to me that it is missing some crucial information.
	 *
	 * Let us add @Bson annotations in the ActorBasic class. These add the field
	 * name references as part of our ActorBasic class, and the CodecRegistry is now
	 * able to see the corresponding structure between the ActorBasic object and the
	 * document fields.
	 *
	 * This basic case of reading from the database is not always close to reality.
	 * As you have learned in earlier lessons, a field in a database can have
	 * different data types, which need to be taken care of. Additionally, sometimes
	 * your app uses data types that are different from what is stored in the
	 * database.
	 *
	 * For this example we will use ActorWithStringId, which as you may have guessed
	 * is an Actor class where the id field is not an ObjectId but a String instead.
	 *
	 * In this case we have several different ways to reach the desired result. The
	 * first option is to write a method that will manually map each document field
	 * to the ActorWithStringId properties.
	 *
	 * Our fromDocument method helps us with that by mapping a Document object into
	 * the ActorWithStringId object and returning the ActorWithStringId object that
	...
	/*
	 * Let us test if our fromDocument mapper works and whether we were able to
	 * successfully map values in each field to the values in the ActorWithStringId
	 * class.
	 */
	...
	public void testMappingDocumentsToActorClass() {
		...
		// Now, we are going to create an ActorWithStringId object by mapping every
		// value
		// in the Document that we just retrieved to the appropriate
		// attribute of the ActorWithStringId class using our helpful fromDocument
		// method.
	...
	/*
	 * Reading seems to work, but if we want to write to the database, we will have
	 * to create a method that does the reverse of fromDocument and manages data
	 * insertion from Actor Class to Document. /
	 * 
	 * /* The above process doesn't seem too terrible because of our simple use
	 * case, but as the complexity and size of our documents grows, this can get
	 * more tedious than needed. There are easier approaches to going from Bson to a
	 * Java Object, so let's take advantage of the simplicity of our case and see
	 * what other options are available to us, namely let's look at custom codec.
	 *
	 * As you may have noticed from the first example of using pojo and a
	 * DefaultCodec, using a Codec allows us to write cleaner code. Theoretically,
	 * we could use a Default Codec again, but that would deprive us of the
	 * opportunity to have a value assigned to the id field due to a type mismatch.
	 * Because now our id field is a String and not an ObjectId.
	 *
	 * As you know _id is the document primary key, which therefore uniquely
	 * identifies a MongoDB document, so it sounds like something we want to have in
	 * our ActorWithStringId object as well. As of today, the default codec doesn't
	 * automatically set the _id at insert time using PojoCodec. So there is a
	 * little more work that needs doing to have a complete transfer of data.
	 *
	 * To accommodate for the missing _id or any cross type conversions we will use
	 * a custom codec, that will help handle our POJO
	 * 
	 * @see mflix.lessons.utils.ActorCodec we provided you with this Codec for our
	 * specific use case. Feel free to take a look and see how it works.
	 *
	 * Now with our Custom Codec in place we can encode a BSON ObjectId straight
	 * into the ActorWithStringId String id value among other things.
	 */
	...
	public void testReadObjectsWithCustomCodec() {
		...
		Bson queryFilter = new Document("_id", actor1Id);
		MongoCollection<ActorWithStringId> customCodecActors = testDb.getCollection("actors", ActorWithStringId.class)
	...
	/*
	 * The question now is: Why use POJO over manually mapping field values into the
	 * object attributes? Well, the first reason for me is that it is a more elegant
	 * implementation, the second is writing. When you use Pojo with a custom codec,
	 * not only do you get to read directly from Bson into your object in a cleaner
	 * way, but you also get to write straight from an object into a Bson
	 * collection. While you can do this with PojoCode, you will have to sacrifice
	 * any type of mismatching between your Class objects and the supported Bson
	 * types, like in our _id field case. So let's look at the CustomCodec example
	 * below to demonstrate the write.
	 */
	...
	/*
	 * Finally, the last method that we wanted to show you, is the one that allows
	 * us to use the default CodecRegistry while customizing the fields that we know
	 * need special treatment. For example, if you have a field that would need type
	 * conversion, or has various different data types in it. This needs to be
	 * addressed when the documents are read into your Java application, here is
	 * another approach to handling it.
	 *
	 * In this approach we use the Default Codec, while applying a custom approach
	 * to the field in question, which in our case is the id field.
	 *
	...
	public void testReadObjectWithCustomFieldCodec() {
		...
		PropertyModelBuilder<String> idPropertyModelBuilder = (PropertyModelBuilder<String>) classModelBuilder
				.getProperty("id");
		...
		CodecRegistry stringIdCodecRegistry = fromRegistries(MongoClientSettings.getDefaultCodecRegistry(),
		...
		MongoCollection<ActorWithStringId> actors = testDb.getCollection("actors", ActorWithStringId.class)
	...
	/*
	 * Let us now recap the advantages and disadvantages of the methods discussed in
	 * today's lesson
	 *
	 * 1. The Pojo implementation is much cleaner, it outsources translation between
	 * BSON and object to a custom codec which makes it easier to manage and
	 * maintain the code.
	 *
	 * 2. In either approach the field names don't have to match the object
	 * attribute name. In case of the custom Codec this is easy to accommodate in
	 * the code, via the @BsonProperty annotation only for the fields that don't
	 * correspond to the attribute name one-to-one.
	 *
	 * 3. To map the Doc to an object manually you have to make use of getters and
	 * setters, while using a POJO allows for automatic introspection (a.k.a
	 * automatic process of analyzing a bean's design patterns to reveal the bean's
	 * properties, events, and methods).
	 *
	 * 4. Handling of generics and subdocuments is cleaner and easier to maintain
	 * with POJO, utilizing the Custom Codec or the field customization approach
	 * instead of writing separate methods to traverse sub-documents with getters
	 * and setters.
	 *
	 * 5. Finally, when writing a Document object to the DB, the document _id is
	 * automatically generated and can be accessed in the Document object for the
	 * subsequent use in the mapping method.
	 *
